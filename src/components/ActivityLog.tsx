...
export default function ActivityLog({ activities, onRead }: ActivityLogProps) {
	const defaultVisible = 5;
	const [visibleActivities, setVisibleActivities] =
		useState<number>(defaultVisible);
	...
	return (
		<div
			className="rounded-lg overflow-x-hidden [scrollbar-color:gray_transparent] [scrollbar-width:thin] h-96 max-h-96 w-1/2 md:min-w-xl min-w-64 border border-solid border-black flex flex-col justify-start relative p-4"
			ref={mainDiv}
			...
			<div className="h-0 sticky bottom-0 flex flex-row justify-center">
				<AnimatePresence>
					{showJumpButton && (
						...
						>
							<span
								className="inline-block transform-[scaleX(1.5)] relative left-[2.5px] -top-0.5 text-white"
								aria-hidden="true"
							>
								^
							</span>
							<span
								className="inline-block transform-[scaleY(0.75)] relative -left-1.25 -top-px text-white"
								aria-hidden="true"
							>
								|
							</span>
